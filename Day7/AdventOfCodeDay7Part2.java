...
public class AdventOfCodeDay7Part2 {
    ...
    private static boolean getCanTotalBeAchievedRec(long[] numbers, long wantedTotal, long currentTotal, int index) {
        ...
        return getCanTotalBeAchievedRec(numbers, wantedTotal, currentTotal + numbers[index], index+1)
                ...
                || getCanTotalBeAchievedRec(numbers, wantedTotal, concatenateNumbers(currentTotal, numbers[index]), index+1);
    }

    private static long concatenateNumbers(long currentTotal, long number) {
        // Concatonate currentTotal to number
        int multiplyBy = 10;
        while(multiplyBy <= number)
            multiplyBy*= 10;
        return currentTotal*multiplyBy + number;
    }
}
