...
public class AdventOfCodeDay5Part2 {
    public static void main(String[] args) throws IOException {
         ...
         *
         * The task is to find which number orderings are incorrect!
         * Then correct them, find the middle element of the sequence (by index, not by value).
         * The answer is the sum of all such elements.
         * */
    ...
    private static int totalSumOfMiddleNumberOfCorrectLists(File file) throws IOException {
         ...
         * Let a helper method process the lists and return either 0 for a correct list or the value of its middle element of a corrected sequence.
        ...
        try(BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(new FileInputStream(file)))) {
            ...
            // Now look at the sequences of numbers, add the middle number of every corrected sequence
            line = bufferedReader.readLine();
            while (line != null) {
    ...
    private static int checkAndGetMiddleElementFromSequence(String sequenceAsString, Map<Integer, List<Integer>> ruleSet) {
        ...
        // For the part 2, fix the numbers until they are correct.
        boolean changesWereMade = true;
        int howManyChanges = 0; // The value doesn't actually matter for this task, this could be another boolean to save if any changes were made, ever, not for current iteration of the loop
        while (changesWereMade) {
            changesWereMade = false; // Reset after every iteration
            ...
            for (int firstNumberIndex = 0; firstNumberIndex < numbers.length; firstNumberIndex++) {
                for (int secondNumberIndex = firstNumberIndex + 1; secondNumberIndex < numbers.length; secondNumberIndex++) {
                    ...
                    if (!needsToBeSwapped(ruleSet, firstNumber, secondNumber))
                        continue;
                    // The ordering is incorrect, swap the elements
        ...
        // Only the incorrect ones need to be returned, if no changes were made, return 0 so nothing is added to the sum
        return howManyChanges == 0 ? 0 : numbers[numbers.length/2];
    }
    private static boolean needsToBeSwapped(Map<Integer, List<Integer>> ruleSet, int firstNumber, int secondNumber) {
        // For every pair of numbers, I want to check if they are in the incorrect order,
        // so I check if second is in the ruleset and first as a value (comes after second number)
